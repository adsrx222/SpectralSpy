...
func Identify(ctx context.Context, db *sql.DB, logger *slog.Logger, fingerprints []SpectralSpy.Fingerprint) (IdentifyResponse, error) {
	if logger == nil {
		logger = slog.Default()
	}
	...
	t4 := time.Now()
	bestSongID, bestScore, matchOffset, confidence := SpectralSpy.MatchFingerprints(fingerprints, dbMap)
	if bestSongID == "" {
		return IdentifyResponse{}, ErrNoMatch
	}
