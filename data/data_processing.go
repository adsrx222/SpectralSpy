...
func processFingerprints(ctx context.Context, db *sql.DB, songID string, fps []SpectralSpy.Fingerprint) error {
	if len(fps) == 0 {
...
func processConstellation(workspacePath, songID string, constellation [][]SpectralSpy.ConstellationPoint) error {
...
func processSamples(ctx context.Context, db *sql.DB, songID, workspacePath string, samples []float64) error {
	fps, constellation := SpectralSpy.ProcessWithPeaks(ctx, samples)
