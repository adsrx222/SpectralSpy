...
func TestProcess_MemoryLeak(t *testing.T) {
	// Generate 2 seconds of silent audio samples
	samples := make([]float64, SAMPLE_RATE*2)

	var m1, m2 runtime.MemStats

	// Warm up to stabilize allocator before measuring
	for i := 0; i < 5; i++ {
		_ = Process(context.Background(), samples)
	}
...
func TestProcess_ContextTimeout(t *testing.T) {
	samples := make([]float64, SAMPLE_RATE*10) // 10 seconds of data
...
func TestDataRace_ConcurrentProcess(t *testing.T) {
	samples := makeSine(SAMPLE_RATE*2, 440)
	const workers = 8
...
func TestDataRace_ConcurrentProcessWithPeaks(t *testing.T) {
	...
	for i := 0; i < workers; i++ {
		go func() {
			_, _ = ProcessWithPeaks(context.Background(), samples)
			done <- struct{}{}
		}()
	}
	for i := 0; i < workers; i++ {
...
func TestDataRace_ConcurrentMatchFingerprints(t *testing.T) {
	fps := Process(context.Background(), makeSine(SAMPLE_RATE, 440))
	...
	for i := 0; i < workers; i++ {
		go func() {
			_, _, _, _ = MatchFingerprints(fps, db)
			done <- struct{}{}
		}()
	}
	for i := 0; i < workers; i++ {
...
// TestProcess_NoCancelledContextGoroutineLeak verifies no goroutines are leaked
// after context cancellation; the goroutine count should stabilize.
func TestProcess_NoCancelledContextGoroutineLeak(t *testing.T) {
	samples := makeSilence(SAMPLE_RATE * 2)

	// Run several cancelled-context calls, then verify goroutines settle back down.
...
// 12. MatchFingerprints Testing Suite (extended)

// TestMatchFingerprints_OffsetMarginConfidence verifies that the returned
// confidence ratio is > 1 when the best song has more votes than the second-best.
func TestMatchFingerprints_OffsetMarginConfidence(t *testing.T) {
	db := map[uint64][]DBEntry{
		10: {{Hash: 10, SongID: "A", AnchorTime: 0.0, Weight: 1.0}},
		11: {{Hash: 11, SongID: "A", AnchorTime: HOP_SIZE, Weight: 1.0}},
		12: {{Hash: 12, SongID: "A", AnchorTime: 2 * HOP_SIZE, Weight: 1.0}},
		20: {{Hash: 20, SongID: "B", AnchorTime: 0.0, Weight: 1.0}},
	}

	// 3 votes for A, 1 for B → confidence > 1
	query := []Fingerprint{
		{Hash: 10, AnchorTime: 0.0},
		{Hash: 11, AnchorTime: HOP_SIZE},
		{Hash: 12, AnchorTime: 2 * HOP_SIZE},
		{Hash: 20, AnchorTime: 0.0},
	}

	song, _, _, confidence := MatchFingerprints(query, db)
	if song != "A" {
...
func TestMatchFingerprints_InsufficientData(t *testing.T) {
	...
	query := []Fingerprint{{Hash: 42, AnchorTime: 5.0}}
	song, score, _, _ := MatchFingerprints(query, db)
	if song != "X" {
...
func TestMatchFingerprints_BoundaryEmptyDB(t *testing.T) {
	query := []Fingerprint{{Hash: 999, AnchorTime: 1.0}}
	song, score, offset, confidence := MatchFingerprints(query, map[uint64][]DBEntry{})
...
func TestMatchFingerprints_BoundaryNilDB(t *testing.T) {
	query := []Fingerprint{{Hash: 1, AnchorTime: 0.0}}
	song, score, _, _ := MatchFingerprints(query, nil)
	if song != "" || score != 0 {
		t.Errorf("expected empty result for nil DB, got song=%q score=%.1f", song, score)
	}
}

// TestMatchFingerprints_CorrectSongIdentification uses the real Process
...
func TestMatchFingerprints_CorrectSongIdentification(t *testing.T) {
	samples := makeSine(SAMPLE_RATE*2, 880)
	...
	song, score, _, _ := MatchFingerprints(fps, db)
	if song != "sine880" {
...
func TestMatchFingerprints(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			song, score, _, _ := MatchFingerprints(tt.query, db)
